...
class JoinSpec extends WordSpec with MustMatchers {
  "Join class" must {
    ...
    "do one-to-one inner joins with implicit conversion" in {
      List(1, 2, 3).oneToOneJoin(List(2, 3), identity[Int], identity[Int]) must be (Iterable((2, 2), (3, 3)))
    }
    ...
    "do one-to-one left joins with implicit conversion" in {
      List(1, 2, 3).oneToOneLeftJoin(List(2, 3), identity[Int], identity[Int]) must be (Iterable((1, None), (2, Some(2)), (3, Some(3))))
    }
  }
}

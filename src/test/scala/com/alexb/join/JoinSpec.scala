...
class JoinSpec extends WordSpec with MustMatchers {
  "Join class" must {
    ...
    "do one-to-one inner joins" in {
      val left = List(1, 2, 3)
      val right = List(2, 3)
      Join.oneToOneJoin(left, right, identity[Int], identity[Int]) must be (Iterable((2, 2), (3, 3)))
    }
    ...
    "do one-to-one left joins" in {
      val left = List(1, 2, 3)
      val right = List(2, 3)
      Join.oneToOneLeftJoin(left, right, identity[Int], identity[Int]) must be (Iterable((1, None), (2, Some(2)), (3, Some(3))))
    }
